  ...
  <div class="bg-white rounded shadow w-2/3 mt-6 overflow-hidden">
    ...
    <div class="px-4 pt-2 flex justify-between text-gray-700 text-sm">
      <div class="flex">
        ...
        <p v-if="post.data.attributes.likes.like_count === 1">
          {{ post.data.attributes.likes.like_count }} like
        </p>
        <p v-else>
      ...
      <div>
        <p v-if="post.data.attributes.comments.comment_count === 1">
    ...
    <div
      ...
      <div class="flex">
        <input v-model="commentBody" type="text" class="w-full pl-4 h-8 bg-gray-200 rounded-lg focus:outline-none" placeholder="Write your comment">
        <button
          v-if="commentBody" 
